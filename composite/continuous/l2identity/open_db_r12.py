...
if __name__ == "__main__":
    ...
    for dirpath, dirnames, filenames in os.walk(db_path):
        ...
        if dirpath.startswith(os.path.join(db_path, "r12_")) and len(dirnames) == 0:
    ...
    tmp = {"path": [], "r12": [], "seed": [], "type": [], "l1f": [], "l2": [], "lf": []}
    ...
    idx15 = df_recos.groupby(['r12', 'seed', 'type'])["RelErr_1.5"].idxmin()
    ...
    times15_comp = best_15[best_15['type'] == 'composite'][["r12", 'time', 'ndcp_time']].values
    times15_blasso = best_15[best_15['type'] == 'blasso'][["r12", 'time']].values
    plt.figure()
    ...
    blasso = best_15[best_15['type'] == 'blasso'].groupby(['r12'])['time'].agg(['median'])
    plt.plot(blasso.index, blasso['median'], label='BLASSO', color='blue', marker='+')
    plt.fill_between(blasso.index, best_15[best_15['type'] == 'blasso'].groupby(['r12'])['time'].quantile(0.25),
                    best_15[best_15['type'] == 'blasso'].groupby(['r12'])['time'].quantile(0.75), alpha=0.2,
                    color='blue')
    compo = best_15[best_15['type'] == 'composite'].groupby(['r12'])[['time', 'ndcp_time']].agg(['median'])
    ...
    plt.fill_between(compo.index, best_15[best_15['type'] == 'composite'].groupby(['r12'])['time'].quantile(0.25),
                    best_15[best_15['type'] == 'composite'].groupby(['r12'])['time'].quantile(0.75), alpha=0.2, color='red')
    plt.fill_between(compo.index, best_15[best_15['type'] == 'composite'].groupby(['r12'])['ndcp_time'].quantile(0.25),
                    best_15[best_15['type'] == 'composite'].groupby(['r12'])['ndcp_time'].quantile(0.75), alpha=0.2, color='red')
    plt.legend()
    plt.show()
